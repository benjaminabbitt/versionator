# C# Patch Test Container
# Tests: versionator emit patch → updates .csproj → dotnet build → run
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/csharp/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/csharp-patch.sh ./test.sh
RUN chmod +x test.sh
