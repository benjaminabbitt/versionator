# Swift Patch Test Container
# Tests: versionator emit patch Package.swift → patched version comment
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/swift/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/swift-patch.sh ./test.sh
RUN chmod +x test.sh
