# Kotlin Emit Test Container
# Tests: versionator emit kotlin → Version.kt → gradle build → run
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/kotlin/emit/ ./
...
COPY tests/containers/scripts/kotlin-emit.sh ./test.sh
