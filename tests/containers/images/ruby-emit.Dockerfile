# Ruby Emit Test Container
# Tests: versionator emit ruby → version.rb → ruby require
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/ruby/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/ruby-emit.sh ./test.sh
RUN chmod +x test.sh
