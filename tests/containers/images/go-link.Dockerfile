# Go Link Test Container
# Tests: versionator link injection via -ldflags
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/go/link/ ./
...
# Copy test script
COPY tests/containers/scripts/go-link.sh ./test.sh
RUN chmod +x test.sh
