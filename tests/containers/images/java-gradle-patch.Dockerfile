# Java Gradle Patch Test Container
# Tests: versionator emit patch build.gradle -> patched version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/java-gradle/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/java-gradle-patch.sh ./test.sh
RUN chmod +x test.sh
