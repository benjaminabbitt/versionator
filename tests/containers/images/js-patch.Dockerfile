# JavaScript Patch Test Container
# Tests: versionator patch → updates package.json → node reads version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/js/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/js-patch.sh ./test.sh
RUN chmod +x test.sh
