# PHP Patch Test Container
# Tests: versionator emit patch → updates composer.json → php reads version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/php/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/php-patch.sh ./test.sh
RUN chmod +x test.sh
