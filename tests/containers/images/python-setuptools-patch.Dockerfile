# Python Setuptools Patch Test Container
# Tests: versionator emit patch → setup.py → verify version
# Requires: docker build -t versionator-builder -f tests/containers/images/versionator-builder.Dockerfile .

# Python runtime with test
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/python-setuptools/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/python-setuptools-patch.sh ./test.sh
RUN chmod +x test.sh
