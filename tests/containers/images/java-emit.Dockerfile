...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/java/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/java-emit.sh ./test.sh
RUN chmod +x test.sh
