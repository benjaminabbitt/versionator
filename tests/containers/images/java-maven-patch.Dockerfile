# Java Maven Patch Test Container
# Tests: versionator emit patch pom.xml -> patched version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/java-maven/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/java-maven-patch.sh ./test.sh
RUN chmod +x test.sh
