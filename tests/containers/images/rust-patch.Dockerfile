# Rust Patch Test Container
# Tests: versionator emit patch → updates Cargo.toml → cargo build → run
# Requires: docker build -t versionator-builder -f tests/containers/images/versionator-builder.Dockerfile .

# Rust runtime with test
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/rust/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/rust-patch.sh ./test.sh
RUN chmod +x test.sh
