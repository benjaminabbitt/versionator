# C# Emit Test Container
# Tests: versionator emit csharp → VersionInfo.cs → dotnet build → run
# Requires: docker build -t versionator-builder -f tests/containers/images/versionator-builder.Dockerfile .

# .NET SDK runtime with test
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/csharp/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/csharp-emit.sh ./test.sh
RUN chmod +x test.sh
