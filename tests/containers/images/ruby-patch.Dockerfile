...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/ruby/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/ruby-patch.sh ./test.sh
RUN chmod +x test.sh
