# Rust Emit Test Container
# Tests: versionator emit rust → src/version.rs → cargo build → run
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/rust/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/rust-emit.sh ./test.sh
RUN chmod +x test.sh
