# JavaScript Emit Test Container
# Tests: versionator emit js → version.js → node import
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/js/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/js-emit.sh ./test.sh
RUN chmod +x test.sh
