# Java Maven Emit Test Container
# Tests: versionator emit java → Version.java → mvn package → run
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/java-maven/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/java-maven-emit.sh ./test.sh
RUN chmod +x test.sh
