...
# Tests: versionator emit build csharp -> /p:Version=X.Y.Z -> dotnet build
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/csharp/link/ ./
...
# Copy test script
COPY tests/containers/scripts/csharp-link.sh ./test.sh
RUN chmod +x test.sh
