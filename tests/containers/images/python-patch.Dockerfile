# Python Patch Test Container
# Tests: versionator emit patch → pyproject.toml → verify version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/python/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/python-patch.sh ./test.sh
RUN chmod +x test.sh
