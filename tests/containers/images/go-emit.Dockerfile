...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/go/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/go-emit.sh ./test.sh
RUN chmod +x test.sh
