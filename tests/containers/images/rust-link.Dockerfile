# Rust Link Test Container
# Tests: versionator emit build rust → VERSION=1.2.3 cargo build → run
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/rust/link/ ./
...
# Copy test script
COPY tests/containers/scripts/rust-link.sh ./test.sh
RUN chmod +x test.sh
