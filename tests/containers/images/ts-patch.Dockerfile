# TypeScript Patch Test Container
# Tests: versionator emit patch → updates package.json → tsc → node reads version
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/ts/patch/ ./
...
# Copy test script
COPY tests/containers/scripts/ts-patch.sh ./test.sh
RUN chmod +x test.sh
