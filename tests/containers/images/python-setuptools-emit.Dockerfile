# Python Setuptools Emit Test Container
# Tests: versionator emit python → _version.py → python import
...
# Copy test project
WORKDIR /test
COPY tests/containers/projects/python-setuptools/emit/ ./
...
# Copy test script
COPY tests/containers/scripts/python-setuptools-emit.sh ./test.sh
RUN chmod +x test.sh
