Feature: End-to-End Integration
  ...
  Scenario: Complete release workflow
    ...
    And the VERSION.json should have version "1.0.1"

  Scenario: Version injection for Python project
    Given a VERSION.json file with version "1.2.3"
  ...
  Scenario: Version injection for Go project
    Given a VERSION.json file with version "0.5.0"
  ...
  Scenario: Pre-release workflow
    Given a VERSION.json file with prefix "v", version "2.0.0" and prerelease "beta"
  ...
  Scenario: Configuration file usage
    Given a config file with:
      ...
      """
    And a VERSION.json file with version "3.0.0"
  ...
  Scenario: Custom variables in template
    Given a VERSION.json file with version "1.0.0" and custom variable "BuildEnv" set to "production"
  ...
  Scenario: Override custom variables via CLI
    Given a VERSION.json file with version "1.0.0" and custom variable "Env" set to "dev"
