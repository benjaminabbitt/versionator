Feature: Version Management
  As a software developer
  I want to manage semantic versions in my project
  So that I can track releases and communicate changes to users

  Background:
    Given a clean git repository
    And a VERSION file with version "1.0.0"

  Scenario: Display current version
    When I run "versionator output version"
    Then the output should be "1.0.0"
    And the exit code should be 0

  Scenario: Display version with prefix
    Given a VERSION file with prefix "v" and version "1.2.3"
    When I run "versionator output version -t '{{Prefix}}{{MajorMinorPatch}}' --prefix"
    Then the output should be "v1.2.3"
    And the exit code should be 0

  Scenario: Increment major version
    When I run "versionator bump major increment"
    And I run "versionator output version"
    Then the output should be "2.0.0"

  Scenario: Increment minor version
    When I run "versionator bump minor increment"
    And I run "versionator output version"
    Then the output should be "1.1.0"

  Scenario: Increment patch version
    When I run "versionator bump patch increment"
    And I run "versionator output version"
    Then the output should be "1.0.1"

  Scenario: Multiple version increments
    When I run "versionator bump patch increment"
    And I run "versionator bump patch increment"
    And I run "versionator bump patch increment"
    And I run "versionator output version"
    Then the output should be "1.0.3"

  Scenario: Major reset resets minor and patch
    Given a VERSION file with version "1.5.3"
    When I run "versionator bump major increment"
    And I run "versionator output version"
    Then the output should be "2.0.0"

  Scenario: Minor reset resets patch
    Given a VERSION file with version "1.5.3"
    When I run "versionator bump minor increment"
    And I run "versionator output version"
    Then the output should be "1.6.0"

  Scenario: Version with custom template
    Given a VERSION file with version "2.1.0"
    When I run "versionator output version -t '{{Major}}.{{Minor}}'"
    Then the output should be "2.1"

  Scenario: Set prefix
    When I run "versionator config prefix set v"
    Then the VERSION should have prefix "v"

  Scenario: Clear prefix
    Given a VERSION file with prefix "v" and version "1.0.0"
    When I run "versionator config prefix disable"
    Then the VERSION should have prefix ""

  Scenario: Set prerelease
    When I run "versionator config prerelease set alpha"
    Then the VERSION should have prerelease "alpha"

  Scenario: Set prerelease with identifier
    When I run "versionator config prerelease set beta.1"
    Then the VERSION should have prerelease "beta.1"

  Scenario: Clear prerelease
    Given a VERSION file with prefix "v", version "1.0.0" and prerelease "alpha"
    When I run "versionator config prerelease clear"
    Then the VERSION should have prerelease ""

  Scenario: Set metadata
    When I run "versionator config metadata set build.123"
    Then the VERSION should have metadata "build.123"

  Scenario: Clear metadata
    Given a VERSION file with prefix "v", version "1.0.0" and metadata "build.456"
    When I run "versionator config metadata clear"
    Then the VERSION should have metadata ""

  Scenario: Version with prerelease
    When I run "versionator config prerelease set rc.1"
    And I run "versionator output version"
    Then the output should be "1.0.0-rc.1"

  Scenario: Version with metadata
    When I run "versionator config metadata set 20241212"
    And I run "versionator output version"
    Then the output should be "1.0.0+20241212"

  Scenario: Version with prerelease and metadata
    When I run "versionator config prerelease set alpha"
    And I run "versionator config metadata set build.1"
    And I run "versionator output version"
    Then the output should be "1.0.0-alpha+build.1"

  Scenario: Incrementing clears prerelease
    Given a VERSION file with prefix "v", version "1.0.0" and prerelease "alpha"
    When I run "versionator bump patch increment"
    And I run "versionator output version"
    Then the output should be "1.0.1"

  # Decrement scenarios
  Scenario: Decrement major version
    Given a VERSION file with version "2.0.0"
    When I run "versionator bump major decrement"
    And I run "versionator output version"
    Then the output should be "1.0.0"

  Scenario: Decrement minor version
    Given a VERSION file with version "1.5.0"
    When I run "versionator bump minor decrement"
    And I run "versionator output version"
    Then the output should be "1.4.0"

  Scenario: Decrement patch version
    Given a VERSION file with version "1.0.5"
    When I run "versionator bump patch decrement"
    And I run "versionator output version"
    Then the output should be "1.0.4"

  Scenario: Decrement using short alias
    Given a VERSION file with version "1.0.5"
    When I run "versionator bump patch decrement"
    And I run "versionator output version"
    Then the output should be "1.0.4"

  Scenario: Cannot decrement below zero
    Given a VERSION file with version "0.0.0"
    When I run "versionator bump major decrement"
    Then the exit code should not be 0

  # Prefix enable/status scenarios
  Scenario: Enable prefix with default value
    When I run "versionator config prefix enable"
    Then the VERSION should have prefix "v"

  Scenario: Prefix status when enabled
    When I run "versionator config prefix set v"
    And I run "versionator config prefix status"
    Then the output should contain "ENABLED"
    And the output should contain "v"

  Scenario: Prefix status when disabled
    When I run "versionator config prefix status"
    Then the output should contain "DISABLED"
